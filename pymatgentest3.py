...
for material_index in range(num_structures):
    ...
    for real_index in range(real_num_structures):
        ...
        real_a, real_b, real_c = real_lattice_lengths[0, real_index]
        real_alpha, real_beta, real_gamma = real_lattice_angles[0,real_index]
        ...
        real_num_atoms_material = real_num_atoms[0, real_index].item()

        real_structure = Structure(real_lattice_vectors, real_atom_types[0, :real_num_atoms[0, real_index].item()], real_frac_coords[0, :real_num_atoms_material])
        match = sm.fit(real_structure, generated_structure)
        if match:  # If structures match
            print(f"RMSD between real {real_index + 1} and generated {material_index + 1} structure (matching)")
        #else:  # If structures don't match
            #if real_num_atoms_material == num_atoms_material:
                #rmsd = custom_rmsd(real_structure, generated_structure)
                #non_matching_rmsd_scores.append(rmsd)
               # print(f"RMSD between real {real_index + 1} and generated {material_index + 1} structure (non-matching): {rmsd:.4f}")

# average_rmsd = sum(rmsd_scores) / len(rmsd_scores)
# min_rmsd = min(rmsd_scores)
# max_rmsd = max(rmsd_scores)

# print(f"Average RMSD: {average_rmsd:.4f}")
