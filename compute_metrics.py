...
from pymatgen.io.cif import CifParser  # Import CifParser from the correct module
...
def visualize_material(structure):
    atomic_symbols = [site.species_string for site in structure.sites]
    atoms = Atoms(positions=structure.cart_coords, symbols=atomic_symbols)
...
def main(args):
    ...
    if "csv" in args.tasks:
        #change the path to the data you want to convert 
        csv_file_path = './HYDRA_JOBS/singlerun/2023-08-04/mp_20/eval_opt.pt'

        crys_array_list, _ = get_crystal_array_list(csv_file_path)
        csv_crys = p_map(lambda x: Crystal(x), crys_array_list)

        structure_data_list = []
        for csv_index, view_crystal in enumerate(csv_crys):
            csv_structure = view_crystal.structure

            csv_structure_data = csv_structure.as_dict()
            csv_structure_json = json.dumps(csv_structure_data)
            structure_data_list.append(csv_structure_json)
        structure_data_df = pd.DataFrame({'cif':structure_data_list})
        csv_filename = "csv_structure_data.csv"
        structure_data_df.to_csv(csv_filename, index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tasks', nargs='+', default=['recon', 'gen', 'opt', 'compare', "csv"])
